...
function Home({
  ...
  const getSubHome = "http://localhost:8000/subhome";
  const categoryMock = "./data/category.json";
  useEffect(() => {
    fetch(getSubHome)
      .then((res) => res.json())
    ...
    <div className="homeWrapper">
      ...
      <div className="categoryShow">
        ...
        <div className="allView">
          <button onClick={goToList}>전체 보기</button>
        </div>
      </div>

      <div className="contentWrapper">
        {categorys.map((el) => {
          const { id, category_image, category_name, category_description } =
            el;
          return (
            <Contentbox
              key={id}
